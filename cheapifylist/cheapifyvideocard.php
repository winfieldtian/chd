...
$toggle = 1;
$query = "SELECT * FROM videocard WHERE name = " . "'" . $_POST['name'] . "'";
$result = mysqli_fetch_array(mysqli_query($con, $query));
$curr = $result;

$thename = $result['name'];
$clock = preg_replace("/[^0-9]/","", $result['clock']);
$memory_size = preg_replace("/[^0-9]/","", $result['memory_size']);
$gpu = preg_replace("/[^0-9]/","", $result['gpu']);
if ($clock == 0 || $memory_size == 0)
	$toggle = 0;

	$cheapifyquery = "SELECT * FROM videocard WHERE price <= " . $result['price'];
	$cheapifyquery = $cheapifyquery . " && (digits(clock) >= " . $clock . " )";
	$cheapifyquery = $cheapifyquery . " && (convert(memory_size, unsigned) >= " . $memory_size . " )";
	$cheapifyquery = $cheapifyquery . " && memory_type = " . "'" . $result['memory_type'] . "'";
	$cheapifyquery = $cheapifyquery . " && interface = " . "'" . $result['interface'] . "'";
	$cheapifyquery = $cheapifyquery . " && (digits(gpu) >= " . $gpu . " )";

if ($result['chipset_manf'] == "NVIDIA")
	$cheapifyquery = $cheapifyquery . " && chipset_manf != 'AMD'";
else
	$cheapifyquery = $cheapifyquery . " && chipset_manf != 'NVIDIA'";
